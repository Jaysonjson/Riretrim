...
public class EnemyBulletZenin : MonoBehaviour, IEnemyBullet
{
    ...
    public void Shoot(GameObject bullet, GSEnemy ship)
    {
       ...
       firstBullet.transform.position = new Vector2(ship.transform.position.x + 0.2f, ship.transform.position.y);
       GameObject secondBullet = Instantiate(bullet, ship.transform);
       secondBullet.transform.position = new Vector2(ship.transform.position.x - 0.2f, ship.transform.position.y);
